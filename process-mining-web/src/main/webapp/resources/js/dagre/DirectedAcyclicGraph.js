function DirectedAcyclicGraph() {
     ...
     */
    var _width = d3.functor("100%");
    var _height = d3.functor("100%");
    var edgeid = function(d) { return d.source.id + d.target.id; };
    var nodeid = function(d) { return d.id; };
    var nodename = function(d) { return d.datum["name"] ? d.datum["name"] : ""; };
    ...
    function graph(selection) {
        selection.each(function(data) {   
            ...
            svg.attr("height", _height.call(this, data));            
    ...
    var layout = function(nodes_d, edges_d) {
        ...
        dagre.layout().nodeSep(90).edgeSep(20).rankSep(60).nodes(nodes_d).edges(edges_d).run(); 
     ...
     */
    graph.width = function(_) { if (!arguments.length) return _width; _width = d3.functor(_); return graph; };
    graph.height = function(_) { if (!arguments.length) return _height; _height = d3.functor(_); return graph; };
